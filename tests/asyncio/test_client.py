...
async def test_transaction(documents_id):
    async with User.objects.use_session():
        ...
        await User.objects.insert_one(
            User.load({"name": "Aber", "age": 18}, partial=True)
        )


async def test_collection_not_initialized():
...
async def test_use_transaction_with_options():
    ...
    async with User.objects.use_transaction(
    ...
    user = await User.objects.find_one(User.name == "Transaction Test")
    assert user is not None
    await User.objects.delete_one(User._id == user._id)


async def test_find_one_and_update_not_found():
    """Test find_one_and_update returning None - covers line 267"""
    # Try to update a non-existent document
    result = await User.objects.find_one_and_update(
        User.name == "NonExistentUser12345", {"$set": {"age": 99}}
    )
    assert result is None


async def test_find_one_and_replace_not_found():
    """Test find_one_and_replace returning None - covers line 311"""
    # Try to replace a non-existent document
    result = await User.objects.find_one_and_replace(
...
async def test_find_one_and_delete_not_found():
    """Test find_one_and_delete returning None - covers line 359"""
    # Try to delete a non-existent document
    result = await User.objects.find_one_and_delete(User.name == "NonExistentUser12345")
    assert result is None


async def test_update_one_to_mongo():
